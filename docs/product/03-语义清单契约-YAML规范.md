...
> 状态：**已废弃**（2026-08 起语义字段内置到寄存器配置，平台自动翻译，不再用 YAML 说明书）
> 保留本文档仅作历史参考。原「固件 agent 产出 YAML → 调试 agent 自己理解」的流程已改为：
> 语义（name/unit/factor/offset/enum）直接存在 `registers` 表，MCP read/write/snapshot 自动翻译（scale ×factor+offset、enum、Q1.15）。
