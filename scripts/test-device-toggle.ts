...
console.log('latest after start:', store.getLatestByObject(obj.id, 'holding-register')[reg.startAddress]?.rawValue)

cfg.toggleObject(obj.id)  // isActive 1 -> 0（断开）
slave.setRegister(0, 999)
await new Promise(r => setTimeout(r, 1500))
const v = store.getLatestByObject(obj.id, 'holding-register')[reg.startAddress]?.rawValue
