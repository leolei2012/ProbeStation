...
const GROUP_MAP: Record<string, string> = { name: 'name', functionCode: 'function_code', startAddress: 'start_address', quantity: 'quantity', mode: 'mode', isActive: 'is_active' }
  ...
  constructor(config: Config) {
    ...
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS register_groups (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        object_id INTEGER NOT NULL, name TEXT NOT NULL, function_code INTEGER DEFAULT 3,
        start_address INTEGER DEFAULT 0, quantity INTEGER DEFAULT 1,
